...
class ApiService {
  ...
  async getMyProgress(): Promise<ApiResponse<any>> {
    return this.request<any>("/cpd/my-progress");
  }

  async getBalance(): Promise<ApiResponse<{ balance: number }>> {
    return this.request<{ balance: number }>("/financial/balance");
  }
  ...
  async getMyEvents(): Promise<ApiResponse<any[]>> {
    return this.request<any[]>("/events/my-events");
  }

  async getMyPolls(): Promise<ApiResponse<any[]>> {
    return this.request<any[]>("/voting/my-polls");
  }
}
